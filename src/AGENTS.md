...
- Motion-quality changes usually cross `src/search/` and `src/runtime/`.
  Put DP/repair mechanics in `src/search/`; put retry orchestration,
  active-set dispatch, Slurm/SSH, and artifact wiring in `src/runtime/`.
- The receiver/RoboDK path consumes the selected path. Do not add hidden
  re-search or quality-polish behavior to wrapper modules or finalization code.
...
- For cross-layer quality edits, compile at least the touched runtime/search
  modules and run the smallest solve or smoke path that exercises the changed
  orchestration.
